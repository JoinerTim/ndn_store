...
export class DashboardController {
    ...
    async getTopProducts(
        ...
        const data = await this.dashboardService.topProducts({
            fromAt,
            toAt,
            storeId: req.staff.store?.id || storeId
        })
    ...
    @Get('/summary/benefitPackageOrder')
    @UseAuth(VerificationJWT)
    async getSummaryBenefitPackage(
        @HeaderParams("token") token: string,
        @Req() req: Request,
        @Res() res: Response,
    ) {
        const data = await this.dashboardService.summaryBenefitPackageTotal()
