...
import { mapReturnedResponse, Request, Response, UseBefore } from "@tsed/common";
...
export const ValidateFile = () => {
    return useDecorators(
        UseBefore(async (req: Request, res: Response, next: any) => {
            console.log('ValidateFile req:', req.files)

            if (isEmpty(req.files)) {
                return res.sendClientError("Không tồn tại file");
            }
            const file = req.files['file'][0] as Express.Multer.File;

            const maxSizeUpload = 20;

            if (file.size > maxSizeUpload * 1024 * 1024) {
                fse.unlinkSync(file.path);
                return res.sendClientError(`Kích thước tối đa của file là ${maxSizeUpload}MB`)
            }
